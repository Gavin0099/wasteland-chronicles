...
const OUT_DIR := "res://artifacts/lunatic-desktop-ui"
...
func _save_view(size: Vector2i, label: String) -> bool:
	...
	if image.save_png(ProjectSettings.globalize_path(path)) != OK:
		push_error("Desktop capture failed: " + path)
		return false
	print("CAPTURED ", path)
	return true
...
func capture() -> void:
	...
	if not result.success:
		push_error("Desktop capture: character creation failed")
		quit(1)
		return
	...
	if not engine.commit_player_intent(world, PlayerIntent.create_accept_quest(world.player.npc_id, "gray_valley_rope_run")).success or not engine.commit_player_intent(world, PlayerIntent.create_buy_item(world.player.npc_id, &"rope", 1)).success or not _reach_dry_well(world, engine):
		push_error("Desktop capture: cannot reach quest turn-in")
		quit(1)
		return
	if not engine.commit_player_intent(world, PlayerIntent.create_turn_in_quest(world.player.npc_id, "gray_valley_rope_run")).success:
		push_error("Desktop capture: quest turn-in failed")
		quit(1)
		return
	shell.refresh_ui()
	shell._show_desktop_scene()
	for size in [Vector2i(1280, 720), Vector2i(1152, 648)]:
		if not await _save_view(size, "dry_well_local"):
			quit(1)
			return
	shell.select_settlement("settlement:new_hope")
	shell.refresh_ui()
	...
	shell.queue_free()
	await process_frame
	var new_hope_world := S1WorldData.create_s1_world()
	var created := engine.commit_character_creation(new_hope_world, Creation.new({
		"source_settlement_id": "settlement:new_hope", "character_name": "新希望旅人", "age": 26,
		"background_id": "FARMER", "trait_ids": []}))
	if not created.success:
		push_error("Desktop capture: New Hope creation failed")
		quit(1)
		return
	var new_hope_shell := PlayableShell.new()
	root.add_child(new_hope_shell)
	new_hope_shell.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	new_hope_shell.setup(new_hope_world, engine)
	for size in [Vector2i(1280, 720), Vector2i(1152, 648)]:
		if not await _save_view(size, "new_hope_local"):
			quit(1)
			return
	new_hope_shell.queue_free()
	await process_frame
	quit(0)
