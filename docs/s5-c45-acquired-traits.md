...
- **Owner constraint:** no combat stat bonus. "Hurt N times -> +5% defence" turns an identity back into a passive buff and re-merges identity with skill. What danger buys here is judgement, not durability, which keeps it in the same knowledge-perk model as the two identities above.
- **Evidence feasibility, checked before the design:** `FIELD_TURN` is recorded for every turn with `{battle_id, turn, command, dealt, taken, hp, enemy_hp}`, so the player's health trajectory is already in the ledger turn by turn; `FIELD_RESULT` carries `{outcome, hp, ...}` with outcomes `VICTORY | ESCAPED | DEAD | CACHE | DEFEAT`, plus `source: "road"` on road battles only. No ledger extension was needed.
- **Evidence:** On two distinct days, a `FIELD_TURN` in which the player took real damage (`taken >= 1`) and was left on exactly one point of health (`hp == 1`), together with a `FIELD_RESULT` on that same day whose outcome is not `DEAD`. Keyed by day, so a long desperate fight is one day at the edge and not five. Standing on 1 HP without being hit does not qualify, a comfortable win does not qualify, and reaching the edge without walking away is not surviving it.
- **Honest asymmetry:** a road fight cannot kill - incoming damage is clamped to leave one point, and being put on one point is what triggers `DEFEAT` and its supply loss. The Gray Valley shed has no such floor. So "one hit from the end" is literal in the shed and partly an artefact of the clamp on the road. Both count, and the difference is stated rather than hidden.
- **Choice:** Accepted at a settlement through the same opt-in authority; qualification alone changes nothing and the identity cannot be taken twice.
- **New knowledge:** At a bandit ambush, before choosing between fighting, paying, talking and running, the screen states the arithmetic of an all-out fight: how many turns to put the bandit down, how much damage arrives in that time, and where that leaves the player. It is derived from the same deterministic `attack_damage` / `enemy_damage` functions the battle itself uses, and the enemy does not swing on the turn it goes down.
- **Limit:** It moves no damage, defence, accuracy, success rate or drop. The suite reads `attack_damage()` either side of the acceptance itself and audits every committed turn against the public combat functions. The forecast assumes the player attacks every turn; defending or fleeing changes the result, and the screen says so.
...
`tests/test_s5_c45_death_tested.gd` exercises the unearned/owned boundary, one day versus two, a five-turn fight counting once, an unhurt turn at 1 HP, a comfortable win, reaching the edge and dying, opt-in and double-acceptance, non-leakage into wreck encounters, checked persistence and two full-world SHA-256 replays. Two gates carry the owner constraint: `attack_damage()` is read either side of the acceptance and must not move, and every committed turn's `dealt` / `taken` is audited against the public combat functions. Two further gates fight the forecast to the end and require it to have predicted the turn count and the ending health exactly, on both a losing and a winning fixture. `tools/capture_death_tested.gd` captures it at 1280x720 and 1152x648.

Remaining C4.5 work: `HARD_BARGAINER`, with a supported effect and real tradeoff or limit; `CARAVAN_FRIEND` is blocked as described above; then player experience testing. This slice does not close C4.5 or all of S5. Three identities now come from three different sources - salvage, helping and danger - which is the set the owner considers sufficient to judge whether receipt-derived identity works at all.
