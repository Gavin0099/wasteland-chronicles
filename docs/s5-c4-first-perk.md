...
Lifetime XP determines Level; it does not grant skill ranks or growth points. The first thresholds are Lv.2 at 20 XP, Lv.3 at 45 XP, and Lv.4 at 75 XP. Existing commissions award 25 XP for the wrench delivery, 20 XP for the rope delivery, and 45 XP for the northern survey. The two ordinary deliveries therefore reach Lv.3 without the survey. The first wreck salvage that actually puts goods or an item in the player's pack awards 10 XP; the first field or road combat victory awards 15 XP. Each is awarded only once per character, recorded in its resolution event, and shown in its result screen. One 25-XP delivery plus both first-time experiences can also reach Lv.3. Skill practice remains a separate progression channel.

Live invariants and checked loading validate tagged experiential XP receipts: source and amount must be paired, match the actual wreck haul or combat victory, and occur no more than once per character. The current player's lifetime XP must cover the sum of those committed awards. Existing quest XP and older saves are not reconstructed from the complete event log, so this does not claim a full lifetime-XP ledger or protection against coordinated rewriting of both a profile and its history.
...
Unowned Perk approaches stay hidden in the player UI and are rejected by the simulation if forged. Selection itself grants no health, combat damage, skill rank, or item. Later Level milestones, further XP sources, more Perks, and acquired Traits are outside this increment.
