...
func execute(params: Dictionary) -> void:
    ...
    if save_error != OK:
        ...
        return
    # An indexed path such as "next_pass:shader_parameter/width" edits another
    # saved file's resource; saving only this one used to lose that edit.
    if not codec.save_touched_external_resources():
        return
