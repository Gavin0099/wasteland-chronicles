...
func _set_layer_name(action: Dictionary) -> bool:
    ...
    if not LAYER_TYPES.has(layer_type) or layer < 1 or layer > 32:
        utils_script.log_error("set_layer_name requires layer_type (one of: %s) and layer from 1 to 32" % ", ".join(LAYER_TYPES.keys()))
        return false
    # Clearing is explicit ("layer_name": ""). A missing key used to clear the name
    # and report success, which is what {"name": "world"} — the key other actions
    # use — silently did.
    if not action.has("layer_name"):
        utils_script.log_error("set_layer_name requires layer_name (the layer's new name; pass \"\" to clear it)")
        return false
