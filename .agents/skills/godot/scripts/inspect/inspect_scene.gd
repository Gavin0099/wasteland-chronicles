...
func execute(params: Dictionary) -> void:
    ...
    if bool(params.get("config_warnings", false)):
        _add_config_warnings(payload, packed_scene as PackedScene, scene_path)
    print(JSON.stringify(payload))

func _add_config_warnings(payload: Dictionary, packed_scene: PackedScene, scene_path: String) -> void:
    # The only part of inspect_scene that needs a live node: configuration
    # warnings are rules over the instantiated tree (the engine registers a
    # body's shape owners when the shape node is parented), so this instantiates
    # the scene, which runs its root script's _init() and stored-property
    # setters exactly as check_project's instantiate pass does.
    var instance: Node = packed_scene.instantiate()
    if instance == null:
        utils_script.log_error(
            "config_warnings needs an instantiable scene, and %s returned null from instantiate() " % scene_path
            + "(see the \"Invalid scene:\" ERROR above). Fix the hierarchy first, or drop config_warnings."
        )
        return
    var findings: Array = node_config_script.collect(instance, {
        "scene": scene_path,
        "local": node_config_script.scene_scope(packed_scene),
        "hints": true,
        "scene_hints": true
    })
    instance.free()
    ...
    for finding in findings:
        ...
        if str(entry["severity"]) == "warning":
            ...
            hints += 1
    payload["config_warnings"] = findings
    payload["config_warning_count"] = warnings
    payload["config_hint_count"] = hints
