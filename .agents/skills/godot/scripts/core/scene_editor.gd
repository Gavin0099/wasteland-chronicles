...
func save_scene(save_path_value: Variant = "") -> bool:
    ...
    return variant_codec.save_touched_external_resources()
...
func _apply_theme_overrides(control: Control, raw_overrides: Variant) -> bool:
    ...
    # Two spellings are accepted: grouped {"constants": {"separation": 12}} and
    # flat {"constants/separation": 12} (the shape Control's own inspector paths
    # use, and the one the help catalog documented). Flat keys fold into groups.
    var grouped := {}
    for raw_key in raw_overrides.keys():
        var key_text := str(raw_key)
        var slash := key_text.find("/")
        if slash > 0 and adders.has(key_text.substr(0, slash)):
            var group_name := key_text.substr(0, slash)
            if not (grouped.get(group_name) is Dictionary):
                grouped[group_name] = {}
            grouped[group_name][key_text.substr(slash + 1)] = raw_overrides[raw_key]
        elif grouped.get(key_text) is Dictionary and raw_overrides[raw_key] is Dictionary:
            grouped[key_text].merge(raw_overrides[raw_key], true)
        else:
            grouped[key_text] = raw_overrides[raw_key]
    raw_overrides = grouped
    ...
    for category in raw_overrides.keys():
        ...
        if not adders.has(category_name):
            ...
            utils_script.log_error("Unsupported theme override category: %s. Use one of %s, grouped ({\"constants\": {\"separation\": 12}}) or flat ({\"constants/separation\": 12})" % [category_name, ", ".join(adders.keys())])
            return false
...
func _load_texture(raw_path: String, context: String) -> Texture2D:
    ...
    # Works today, breaks tomorrow: the saved scene points at the image by path,
    # and a fresh process has no loader for an image that was never imported.
    utils_script.log_info("[WARN] %s has no .import sidecar yet, so it was read as a raw image. The file this operation saves references it by path, and nothing can load that reference until the project is imported. Run: godot --headless --path <project> --import" % texture_path)
