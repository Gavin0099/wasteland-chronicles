...
var node_config_script = preload("../core/node_config_rules.gd")
...
# Node configuration warnings (the editor's yellow triangles, re-implemented in
# core/node_config_rules.gd because get_configuration_warnings() is not bound
# outside the editor) and the project-wide physics layer/mask survey. Both ride
# along on the instantiate pass, so both are silently off without it.
var _config_warnings_enabled: bool = true
var _physics_layers_enabled: bool = true
var _config_findings: Array = []
var _physics_accumulator: Dictionary = {}
# Scenes another scene instantiates: whole-scene hints ("no Camera3D") are
# meaningless for a prop that is always placed inside a level.
var _instanced_scenes: Dictionary = {}

# core/error_capture_logger.gd when the engine has Logger (Godot 4.5+), else null.
# A shader that fails to compile raises nothing in-process — load() and the
# material assignment both succeed — so this is the only way to fail the file
# here instead of leaving it to whoever parses stderr afterwards.
var _error_logger: Object = null

func execute(params: Dictionary) -> void:
    _install_error_logger()
    _instantiate_scenes = bool(params.get("instantiate", true))
    _config_warnings_enabled = bool(params.get("config_warnings", true)) and _instantiate_scenes
    _physics_layers_enabled = bool(params.get("physics_layers", true)) and _instantiate_scenes
    _physics_accumulator = node_config_script.new_physics_layer_accumulator()
    ...
    if _config_warnings_enabled:
        _survey_instanced_scenes(project_path)
    ...
    var config_warnings := _report_config_findings()
    var result = {
        ...
        "config_warnings_enabled": _config_warnings_enabled,
        "config_warnings": config_warnings,
        "config_warning_count": _count_severity(config_warnings, "warning"),
        "config_hint_count": _count_severity(config_warnings, "hint")
    }
    if _physics_layers_enabled:
        result["physics_layers"] = node_config_script.finish_physics_layers(
            _physics_accumulator, node_config_script.read_layer_names()
        )

    _remove_error_logger()
    utils_script.log_info(
        "Check complete. Checked " + str(checked) + " files, " + str(failed.size()) + " failed to load."
    )
    print(JSON.stringify(result))

func _install_error_logger() -> void:
    # Loaded dynamically and driven through call(): `extends Logger` and
    # OS.add_logger do not exist before 4.5, and this op has to keep parsing there.
    if not ClassDB.class_exists(&"Logger") or not OS.has_method(&"add_logger"):
        return
    var logger_path: String = str(get_script().resource_path).get_base_dir().path_join("../core/error_capture_logger.gd")
    var logger_script: Variant = load(logger_path)
    if logger_script is GDScript and logger_script.can_instantiate():
        _error_logger = logger_script.new()
        OS.call(&"add_logger", _error_logger)

func _remove_error_logger() -> void:
    if _error_logger != null:
        OS.call(&"remove_logger", _error_logger)
        _error_logger = null

func _shader_error_count() -> int:
    if _error_logger == null:
        return 0
    var captured: Array = _error_logger.get(&"shader_errors")
    return captured.size()

func _shader_errors_since(count_before: int) -> Array:
    if _error_logger == null:
        return []
    var captured: Array = _error_logger.get(&"shader_errors")
    return captured.slice(count_before)

func _survey_instanced_scenes(base_path: String) -> void:
    # Cheap header parse per scene: a scene some other scene instantiates (or
    # inherits from) is a component, not a level, so it is not expected to carry
    # a camera or a light of its own.
    var scenes := PackedStringArray()
    scenes.append_array(uid_utils_script.find_files(base_path, ".tscn"))
    scenes.append_array(uid_utils_script.find_files(base_path, ".scn"))
    for scene_path in scenes:
        for entry in ResourceLoader.get_dependencies(scene_path):
            var dependency := _dependency_path(str(entry))
            if dependency.ends_with(".tscn") or dependency.ends_with(".scn"):
                _instanced_scenes[dependency] = true

func _collect_node_config(path: String, packed: PackedScene, instance: Node) -> void:
    var scope: Dictionary = node_config_script.scene_scope(packed)
    if _config_warnings_enabled:
        var findings: Array = node_config_script.collect(instance, {
            "scene": path,
            "local": scope,
            "hints": true,
            "scene_hints": not _instanced_scenes.has(path)
        })
        for finding in findings:
            var entry: Dictionary = finding
            entry["scene"] = path
            _config_findings.append(entry)
    if _physics_layers_enabled:
        node_config_script.collect_physics_layers(instance, path, scope, _physics_accumulator)

func _report_config_findings() -> Array:
    # One WARNING line per editor-parity finding so godot_log_parser.py files it
    # under category "node_config" with the scene as `file`; the indented "fix:"
    # continuation becomes its suggested_fix. Hints are payload-only: they are
    # heuristics and must never fail a run.
    var reported: Array = []
    var seen: Dictionary = {}
    for entry in _config_findings:
        var key := "%s|%s|%s" % [str(entry["scene"]), str(entry["node_path"]), str(entry["rule"])]
        if seen.has(key):
            continue
        seen[key] = true
        reported.append(entry)
        if str(entry["severity"]) != "warning":
            continue
        print("WARNING: [node_config:%s] %s::%s: %s" % [
            str(entry["rule"]), str(entry["scene"]), str(entry["node_path"]),
            str(entry["message"]).replace("\n", " ")
        ])
        print("   fix: " + str(entry["fix"]))
    return reported

func _count_severity(entries: Array, severity: String) -> int:
    var total := 0
    for entry in entries:
        if str(entry["severity"]) == severity:
            total += 1
    return total
...
func _load_failure_reason(path: String, kind: String) -> String:
    ...
    if kind == "shader" and resource is Shader:
        ...
        var errors_before := _shader_error_count()
        var material := ShaderMaterial.new()
        material.shader = resource
        # The compile is synchronous with the assignment (verified on 4.7 under
        # the headless dummy renderer), so anything new in the logger is this file's.
        var compile_errors := _shader_errors_since(errors_before)
        if not compile_errors.is_empty():
            var first: Dictionary = compile_errors[0]
            var line_number: int = int(first[&"line"])
            var where := (" at line %d" % line_number) if line_number > 0 else ""
            return "shader compile error%s: %s (a shader that does not compile renders as the default material with no runtime error)" % [where, str(first[&"message"])]
    ...
    if kind == "scene" and _instantiate_scenes and resource is PackedScene:
        ...
        if _config_warnings_enabled or _physics_layers_enabled:
            _collect_node_config(path, resource as PackedScene, instance)
