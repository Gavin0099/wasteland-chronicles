...
func execute(params: Dictionary) -> void:
    ...
    if append_error != OK:
        ...
        root.free()
        return
    # glTF carries 3D geometry only. A 2D scene used to "export" fine: exit 0, a
    # 356-byte file with no meshes and an empty buffer — which then breaks every
    # later --import when it sits inside res://.
    var mesh_count: int = state.get_meshes().size()
    var exported_node_count: int = state.get_nodes().size()
    if mesh_count == 0:
        utils_script.log_error("gltf_export: %s (node %s) contains no 3D meshes, so there is nothing to export — glTF carries MeshInstance3D/CSG/GridMap geometry, not 2D nodes. Nothing was written" % [scene_path, node_path])
        root.free()
        return
